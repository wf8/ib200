...
library(optimx)         
# You need to have some version of optimx available as it is a BioGeoBEARS dependency; however, if you don't want to use optimx, and use optim() (from R core) you can set: BioGeoBEARS_run_object$use_optimx = FALSE
...
# crucial to fix bug in uppass calculations
...
#wd = np("~")
#setwd(wd)
setwd ("/Users/mala/Desktop/Lab21")
...
# In summary, you should collapse multiple specimens into species/lineages if data indicates they are the same genetic population.
######################################################
...
# If you have more than about 500-600 states, the calculations will get REALLY slow,since the program has to exponentiate a matrix of e.g. 600x600.  Often the computer will just sit there and crunch, and never get through the calculation of the first likelihood.
# 
...
# In a Bayesian analysis, it might work OK, but the ML point estimates are not identical.
...
# You will get two warning messages at the end of the run.
#######################################################
# Run DEC+J
#######################################################
BioGeoBEARS_run_object = define_BioGeoBEARS_run()
...
# Question 1: Neat! Please send me this pdf so that I know you were able to get this to run. Do you notice any differences between the DEC and the DEC+J?
#######################################################
# Statistics
#######################################################
...
# The null hypothesis for a Likelihood Ratio Test (LRT) is that two modelsconfer the same likelihood on the data. See: Brian O'Meara's webpage:
...
cmdstr = paste("open ", pdffn, sep="")
system(cmdstr)
# Question 2: Send me this pdf as well. What are the differences between DIVALIKE and DIVALIKE+J? What about compared to the DEC and DEC+J?
#######################################################
# Stats
#######################################################
# We have to extract the log-likelihood differently, depending on the version of optim/optimx
