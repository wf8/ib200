...
#
# I'll post his full tutorial online for those who would like to explore picante further.
#
...
#Let's take a look at the elements of this list.  This is the made up phylogenetic tree for our example. Let's rename it 'phy'
phy <- phylocom$phylo
plot(phy)
...
# The rows are the communities.  The first four columns should be pretty straight forward given the definition of the NRI from today’s lecture.  To review:
...
# mpd.obs.z Standardized eect size of mpd vs. null communities (equivalent to-NRI)
...
#
#
# QUESTION 4:  Look at the column labeled 'K'. This is Bloomberg's K that we have previously discussed in lecture.  What do these numbers mean?  Which traits have strong phylogenetic signal?  Do these values agree with your previous predictions?
#
#################################################################################
#  Reading Data
#################################################################################
...
sample.oak <- read.table('oaksample.txt')
sample.oak <- sample2matrix(sample.oak)
sample.oak
